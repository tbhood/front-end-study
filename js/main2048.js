...
// --------------------------------------------------------------------------------------------------------------------
var board = new Array();//存储4*4格子的数字
var score = 0;//得分
...
function gameover() {
    ...
    var conFirmMsg = confirm("游戏结束！您的得分为：" + score + "\n是否需要继续游戏？\n是：继续游戏  否：提交成绩")
    if(conFirmMsg){
    	newgame();
    }else{
    	var promMsg = prompt("请输入您的名字！（不能为空）");
    	if(promMsg != null && promMsg != ""){
    		var data = '<tr><td>'+promMsg+'</td><td class="history-score">'+score+'</td><td class="history-place">'+1+'</td></tr>';
    		$("tbody").append(data);
    		sortTable();
    	}else{
    		alert("名字不能为空，出错，无法提交！");
    	}
...
/*动态排序表格*/
function sortTable(){
	var scoreArr = $("#history-table").find('tbody > tr').get();
	scoreArr.sort(function(a, b){
		var col1 = $(a).children('td').eq(1).text();
		var col2 = $(b).children('td').eq(1).text();
		return col1 < col2 ? 1 : -1;//从大到小排序
	});
	scoreArr.forEach(function(val,index){
		$(val).children('td').eq(2).text(index+1);//添加名次
	});
	$.each(scoreArr, function(i,row) {
		$("tbody").append(scoreArr);
	});
